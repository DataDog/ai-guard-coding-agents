...
The installer is published as a signed release asset, so you can verify it with the [GitHub CLI](https://cli.github.com) before running it: 
...
- **GitHub CLI**: `gh` *optional* but *recommended*, used to verify the binary's signature
...
Removes the AI Guard hooks from your coding agent config, deletes the binary and `config.env`, and clears the
